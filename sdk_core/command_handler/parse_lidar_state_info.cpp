                                         ...
                                         std::set<ParamKeyName>& key_mask)
{
  ...
  //     info.host_state_info.host_ip_addr, info.host_state_info.host_state_info_port,
  //     info.host_state_info.lidar_state_info_port);
  ...
  // std::string version_hardware = std::to_string(info.version_hardware[0]) + ":" +
  // std::to_string(info.version_hardware[1]) + ":" +
  ...
  // printf("Lidar state info, cur_work_state:%u, core_temp:%d, powerup_cnt:%u, local_time_now:%lu, last_sync_time:%lu,
  // time_offset:%ld.\n",
  //     info.cur_work_state, info.core_temp, info.powerup_cnt, info.local_time_now, info.last_sync_time,
  //     info.time_offset);
...
void ParseLidarStateInfo::ParseImuDataHostIpCfg(const CommPacket& packet, uint16_t off, DirectLidarStateInfo& info)
{
  ...
  std::string host_imu_data_ip_str = std::to_string(host_imu_data_ip[0]) + "." + std::to_string(host_imu_data_ip[1]) +
                                     "." + std::to_string(host_imu_data_ip[2]) + "." +
...
{
  std::string ip_str = std::to_string(packet.data[off]) + "." + std::to_string(packet.data[off + 1]) + "." +
                       std::to_string(packet.data[off + 2]) + "." + std::to_string(packet.data[off + 3]);
...
void ParseLidarStateInfo::LivoxLidarStateInfoToJson(const DirectLidarStateInfo& info,
                                                    const std::set<ParamKeyName>& key_mask, std::string& lidar_info)
{
