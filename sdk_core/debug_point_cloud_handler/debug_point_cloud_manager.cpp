...
void DebugPointCloudManager::AddDevice(const uint32_t handle, const DetectionData* detection_data)
{
  ...
  {
    std::string ip = fmt::format("{}.{}.{}.{}", detection_data->lidar_ip[0], detection_data->lidar_ip[1],
                                 detection_data->lidar_ip[2], detection_data->lidar_ip[3]);
    devices_info_.emplace(
        handle, LidarDeviceInfo{ detection_data->sn, detection_data->dev_type, ip, detection_data->cmd_port });
  }
}
...
void DebugPointCloudManager::Handler(uint32_t handle, uint16_t lidar_port, uint8_t* buf, uint32_t buf_size)
{
  ...
  if (handlers_.find(handle) == handlers_.end())
  {
    ...
    {
      handlers_.emplace(handle,
                        std::make_shared<DebugPointCloudHandler>(handle, it->second.sn, it->second.dev_type, path_));
