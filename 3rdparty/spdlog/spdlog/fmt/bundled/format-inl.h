...
FMT_FUNC bool grisu2_round(char* buf, int& size, int max_digits, uint64_t delta, uint64_t remainder, uint64_t exp,
...
FMT_FUNC bool grisu2_gen_digits(char* buf, int& size, uint32_t hi, uint64_t lo, int& exp, uint64_t delta, const fp& one,
  ...
  while (exp > 0)
  {
    ...
    if (remainder <= delta || size > max_digits)
    {
      return grisu2_round(buf, size, max_digits, delta, remainder,
                          static_cast<uint64_t>(data::POWERS_OF_10_32[exp]) << -one.e, diff.f, exp);
    }
  }
...
template <typename Double>
FMT_FUNC typename std::enable_if<sizeof(Double) == sizeof(uint64_t), bool>::type grisu2_format(Double value,
                                                                                               buffer& buf,
