...
{
  FMT_ASSERT(value >= (std::numeric_limits<int>::min)() && value <= (std::numeric_limits<int>::max)(), "invalid value");
...
struct chrono_formatter
{
  ...
  explicit chrono_formatter(FormatContext& ctx, OutputIt o) : context(ctx), out(o)
  {
  }

  int hour() const
  {
    return to_int((s.count() / 3600) % 24);
  }
  ...
  int minute() const
  {
    return to_int((s.count() / 60) % 60);
  }
  int second() const
  {
    return to_int(s.count() % 60);
  }
...
public:
  ...
  auto format(const duration& d, FormatContext& ctx) -> decltype(ctx.out())
  {
    ...
    if (begin == end || *begin == '}')
    {
      ...
      internal::handle_dynamic_spec<internal::width_checker>(spec.width_, width_ref, ctx);
    }
    else
    {
